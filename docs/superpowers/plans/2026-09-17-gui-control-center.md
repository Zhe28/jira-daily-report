...
- **Task 6**：三处偏离计划代码块：(a) actix 4.15 中 `build_app` 实际返回类型必须写 `App<impl ServiceFactory<ServiceRequest, Config=(), Response=ServiceResponse, Error=Error, InitError=()>>`——裸 `App` 与 `App<()>` 都因 `IntoServiceFactory` 约束不满足而 E0277（`App` 的 type alias 与 `init_service` 期望的 `Config=AppConfig` 不匹配），该返回类型签名已写入 mod.rs 注释；(b) `InnerCtx.ai/store` 用 `Arc<RwLock<Arc<dyn ...>>>` 双层包装——`std::sync::RwLock` 非 `Send`/`Clone`，无法直接进 `web::block` 闭包；(c) `config_put` 里客户端重建与替换放入 `web::block`——旧 `reqwest::blocking::Client` 内部持有 tokio runtime，在 async 任务内 drop 会 panic（"Cannot drop a runtime in a context where blocking is not allowed"）。`Config`/`Repo` 加 `Serialize` + `Option` 字段 `skip_serializing_if` 按计划（TOML 序列化保持可选字段省略，行为不变）。
